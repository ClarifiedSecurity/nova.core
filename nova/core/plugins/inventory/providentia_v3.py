...
class InventoryModule(BaseInventoryPlugin):
    ...
    async def fetch_from_providentia(self, endpoint=""):
        ...
        # The rest of the function will try to use caching with ETag and local cache file
        # This will not re-download the inventory endpoint if it has not changed since last fetch
        # Filenames for caching
        etag_filename = f"/tmp/providentia_{self.project}_{endpoint.replace('/', '')}.etag"
        cache_filename = f"/tmp/providentia_{self.project}_{endpoint.replace('/', '')}.json"

        # Load existing ETag if available
        ...
        async with self._session.get(url, headers=headers) as response:
            if response.status == 200:
                ...
                with open(cache_filename, "w") as f:
                    json.dump(api_endpoint_response, f, indent=2)
                ...
                if os.path.exists(cache_filename):
                    with open(cache_filename, "r") as f:
                        try:
                            ...
                            raise AnsibleParserError(f"Corrupted cache file {cache_filename} has been deleted, try again")
                else:
                    os.remove(etag_filename)
                    raise AnsibleParserError("Got 304 Not Modified but no local inventory cache exists, removing local ETag, try again")
